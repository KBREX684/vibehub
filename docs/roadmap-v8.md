...
- **广场**只有时间 + 热度排序，没有关注流 / 推荐流；用户粘性靠运气
- **项目画廊**曝光权重只有手工 `featuredRank`，没有基于互动的真实排序
...
- 数据库：**不新增核心表**，复用已有 AgentBinding / AgentActionAudit / AgentConfirmationRequest / AutomationWorkflow / AdminAiSuggestion；只做字段级补充（如 TeamAgentRole）
...
1. 切 `feature/v8` 主干 · 冻结 `release/v7.0-rc1` 只允许 hotfix
2. 启动 W6（中国支付商务）· 这是唯一卡外部时间的项
3. 启动 W1 + W2 · 视觉成熟度最快见效
4. 启动 W3 · TeamAgentMembership 迁移 + API 骨架 + 前端 MVP
5. 启动 W7 · 运营仪表盘框架 + AI 审核摘要 MVP
6. 启动 W8 · Redis 灰度

六件事在 Phase Alpha 末要有可审阅产物。
...
- Redis 拓扑未上线
- Agent 协作总线（TeamAgentMembership + 角色牌 + 团队侧 UI + 跨用户协作）未落地
- 首屏叙事仍未传达 AI+Human 定位
- 设计系统组件层割裂
- 运营仪表盘 + AI 审核闭环未跑通
