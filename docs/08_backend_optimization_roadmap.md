...
3. ⬜ **每用户写操作速率限制**（当前只有 per-key 限制，需要 per-user per-tool 细粒度限制）
4. ⬜ **写操作幂等性设计**（同一 Agent 重试时不产生重复数据，需 `idempotencyKey` 参数）
5. ⬜ **内容安全过滤**（写入帖子/评论时触发关键词过滤，不能只靠人工审核）
6. ⬜ **`write:posts`、`write:projects` 等 scope 注册**（`api-key-scopes.ts` 中添加）
7. ⬜ **MCP 写工具的 OpenAPI schema 在实现前更新**（禁止先上线后补文档）
