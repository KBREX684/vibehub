...
export default async function EditProjectPage({ params }: Props) {
  ...
  if (!session) redirect(`/login?redirect=/projects/${encodeURIComponent(slug)}/edit`);
  ...
  if (!creator || creator.userId !== session.userId) {
    return (
      <main className="container max-w-xl pb-24 pt-8 space-y-6">
        <Link href={`/projects/${encodeURIComponent(slug)}`} className="inline-flex items-center gap-1.5 text-sm text-[var(--color-text-secondary)]">
          <ArrowLeft className="w-4 h-4" />
          Back to project
        </Link>
        <div className="card p-8">
          <h1 className="text-xl font-bold text-[var(--color-text-primary)] m-0">Forbidden</h1>
          <p className="text-sm text-[var(--color-text-secondary)] mt-2 mb-0">Only the project creator can edit this page.</p>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="container max-w-2xl pb-24 pt-8 space-y-6">
      ...
      <div className="flex items-center gap-3 pb-5 border-b border-[var(--color-border)]">
        ...
        <div>
          <h1 className="text-xl font-bold text-[var(--color-text-primary)] m-0">Edit project</h1>
