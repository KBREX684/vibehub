...
export default async function AdminEnterprisePage() {
  ...
  const rows = await Promise.all(
    items.map(async (item) => ({
      ...item,
      adminAi: await getOrCreateEnterpriseAi({
        userId: item.userId,
        ...
        {rows.map((item) => (
          <article key={item.userId} className="card p-5 space-y-3">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-[var(--color-text-muted)] m-0 mt-1">
                  Applicant: {item.userId} · Website: {item.organizationWebsite || "—"}
                </p>
              </div>
            ...
            <div className="rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-bg-elevated)] p-3">
              ...
              <p className="text-xs text-[var(--color-text-secondary)] mt-1 mb-0">{item.adminAi.suggestion}</p>
              <p className="text-[10px] text-[var(--color-text-muted)] mt-2 mb-0">
                Risk: {item.adminAi.riskLevel} · Confidence: {item.adminAi.confidence?.toFixed(2) ?? "n/a"}
              </p>
              <p className="text-[10px] text-[var(--color-text-muted)] mt-1 mb-0">
                Queue: {item.adminAi.queue ?? "enterprise-verification"} · Priority: {item.adminAi.priority ?? "normal"}
              </p>
              {item.adminAi.labels?.length ? (
                <div className="tag-row mt-2">
                  {item.adminAi.labels.map((label) => (
