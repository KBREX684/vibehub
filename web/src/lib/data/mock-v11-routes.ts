 ...
 *
 * NOTE (v11 merge fix-up): the original FE PR put these helpers in
 * `mock-persist.ts`, which collided with the v10 in-memory persistence
 * helpers (`createPersistedArray`, `resetPersistedMockStore`) used by
 * the backend repositories. They have been split here so both layers
 * can co-exist.
 */
