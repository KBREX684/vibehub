...
export async function anchorLedgerBundleWithZhixin(
  input: AnchorLedgerBundleInput
): Promise<LegalAnchorReceipt> {
  const endpoint = env("ZHIXIN_LEDGER_ANCHOR_ENDPOINT");
  const apiKey = env("ZHIXIN_LEDGER_ANCHOR_API_KEY");
  if (!endpoint || !apiKey) {
    if (process.env.NODE_ENV === "production") {
      throw new RepositoryError("INTERNAL", "至信链锚定服务未配置", 503);
    }
    return createFallbackReceipt("zhixin", input, "至信链锚定");
  }
  ...
  if (!response.ok) {
    throw new RepositoryError("INTERNAL", `至信链锚定失败 (${response.status})`, 502);
  }
  ...
  if (!data.txId || !data.href) {
    throw new RepositoryError("INTERNAL", "至信链锚定返回无效结果", 502);
  }
